import swedbankLogo from "../logos/swedbanklogo.png";
import icaLogo from "../logos/icabankenlogo.png";
import visaLogo from "../logos/visalogo.png";
import { useSelector, useDispatch } from "react-redux";
import { changeActive } from "../redux/slices/walletSlice";
//import { useState } from "react";

const Wallet = () => {
  const dispatch = useDispatch();
  const listOfCards = useSelector((state) => state.walletList.listOfCards);
  const { inActiveCards } = useSelector((state) => state.walletList);
  //const { isActive } = useSelector((state) => state.walletSlice.isActive);

  let onChangeActive = () => {
    console.log(listOfCards);
    console.log(inActiveCards);
    //dispatch(changeActive());
  };

  /*const testOnClick = () => {
    {
      listOfCards.map((card) => {
        if (card.id == 1) {
          card.ifActive = true;
        } else if (card.id == 2 || card.id == 3 || card.id == 4) {
          card.ifActive = false;
        }
      });
    }
  };*/

  return (
    <div>
      {listOfCards.map((card, i) => {
        if (card.ifActive === true) {
          return (
            <div key={i} className="card">
              <div className="whoKnows"></div>
              <div className="logo">
                <img src={visaLogo} />
              </div>
              <div className="chip">
                <img
                  src="data:image/svg+xml;base64,"
                  alt=""
                />
              </div>
              <div className="cardNumbers">
                <p>{card.cardNumber}</p>
              </div>
              <div className="name">
                <p>
                  {card.firstName} {card.lastName}
                </p>
              </div>
              <div className="date">
                <p>
                  {card.validMonth}/{card.validYear}
                </p>
              </div>
            </div>
          );
        }
      })}

      <h3>Inactive cards</h3>
      <div>
        {/* Lägg kort här som kommer från add new card */}
        {inActiveCards.map((card, i) => {
          return (
            <div key={i} className="card" onClick={onChangeActive}>
              {/* onClick={} */}
              <div className="whoKnows"></div>
              <div className="logo">
                <img src={visaLogo} />
              </div>
              <div className="chip">
                <img
                  src="data:image/svg+xml;base64,"
                  alt=""
                />
              </div>
              <div className="cardNumbers">
                <p>{card.cardNumber}</p>
              </div>
              <div className="name">
                <p>
                  {card.firstName} {card.lastName}
                </p>
              </div>
              <div className="date">
                <p>
                  {card.validMonth}/{card.validYear}
                </p>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
};

export default Wallet;

/*
fixa med siffrorna
fixa så man kan byta mellan det aktiva kortet
fixa olika kort och en drop down list
fixa så det aktiva kortet inte är "hårdkodat" utan den ska också ingå i arrayen,
kanske den som ska vara vårt initial state? 
*/
